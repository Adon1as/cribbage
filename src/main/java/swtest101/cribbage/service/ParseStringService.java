...
public class ParseStringService {
	...
	public static Card parseToCard(String cardStr) {
		if(cardStr.length()!=2) {
			throw new InvalidRankException(INVALID_CARD_STRING_ERROR_MSG + cardStr);
		}
	...
	public static List<Card> parseToListOfCards(String handOfCards) {
		...
		for(int i = 0; i < handOfCards.length(); i = i + 2) {
			
			String sub =  handOfCards.substring(i,i+2);
			cards.add(parseToCard(sub));
